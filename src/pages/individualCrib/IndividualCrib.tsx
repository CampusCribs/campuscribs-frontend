...
const IndividualCrib = () => {
  ...
  return (
    <div className="mb-6">
      ...
      <div>
        <IndividualSlider images={{data && data.data.images}} />
      </div>
      <div className="flex ">
        <div>
          <img
      ...
      <div>
        <div className="text-3xl font-semibold p-4">{data && data.data.title}</div>
      ...
      <div className="flex flex-row-reverse px-8 py-2">
        <button
          ...
          onClick={() => navigate(`/profile/${data && data.data.username}`)}
        >
          Contact
        </button>
      </div>
